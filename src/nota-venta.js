...
//
// Los precios del sistema incluyen IVA. En la nota se muestran netos:
//   precio unitario neto = precio con IVA / 1,19
//   total neto de línea  = cantidad × precio con IVA / 1,19
//   TOTAL NETO           = suma de los totales netos de línea
//   TOTAL A PAGAR        = suma de cantidad × precio con IVA (lo que paga el cliente)
//   I.V.A. 19%           = TOTAL A PAGAR − TOTAL NETO
// Así la columna de totales suma exactamente el neto y el total a pagar no cambia por redondeos.
...
export async function buildNotaVenta(pedidoId) {
  ...
  const rows = lineas.map(l => {
    ...
    return {
      ...
      codigo: l.codigo,
      marca: l.marca,
      unitario: money(Number(l.venta) / (1 + IVA)),
      total: money(bruto / (1 + IVA)),
      netoLinea: Math.round(bruto / (1 + IVA)),
      bruto,
    };
  });
  const total = Math.round(rows.reduce((a, r) => a + r.bruto, 0));
  const neto = rows.reduce((a, r) => a + r.netoLinea, 0);
